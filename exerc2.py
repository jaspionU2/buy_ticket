...
def ticketsAvailable(listEvent=[], numTickets=None, nameEvent=""):
    ...
    with open("mockEvents.json", "r") as data:
        data = json.load(data)
    ...
    for event in data:
        if event["nome_evento"] == nameEvent:
            if event["quantIngressos"] >= numTickets and numTickets > 0:
                ...
                with open("mockEvents.json", "w") as events:
                    json.dump(data, events, indent=4)
                return numTickets
            else:
                return -1
    return -1
...
def buyTicket(listEvent=[], orders=[], nameEvent="", nameBuyer="", numTickets=None):
    ...
    for event in listEvent:
        if event["nome_evento"] == nameEvent:
            ...
            while quantIngressos == -1:
                print(
                    "ERRO: A quantidade de ingressos informada, não esta disponivel para o evento.Informe uma quantidade valida.\n"
                )
                quantIngressos = ticketsAvailable(listEvent, numTickets, nameEvent)

            cuponDescount = applyDescount(event["valor"], quantIngressos)
            price = cuponDescount.checkDescount()

            if cuponDescount.descountApllied():
                print(
                    f"Desconto de {cuponDescount.typeCoupon} foi aplicado sobre o valor de {cuponDescount.totPrice}"
                )
            
            dataCompra = date.today()

            with open("mockBuyers.json", "r") as buyers:
               try:
                   orders = json.load(buyers)
            ...
            with open("mockBuyers.json", "w") as buyers:
                json.dump(orders, buyers, indent=4)
               
            print(
                f'ingresso(s) para o evento {event["nome_evento"]} comprado com sucesso!\n'
                )
    ...
    if not eventoEncontrado:
        eventoIngresso = input(
            "Evento informado é invalido. Digite [S] para digitar novo evento ou [N] para interromper a compra: "
        ).lower()
        ...
        match switchController:
            case 1:
                ...
                while not eventoValido:
                    eventoIngresso = input("Informe o evento para o qual deseja comprar ingressos: ").lower()
                    if buyTicket(listEvents, infoBuyer, eventoIngresso):
                        break
